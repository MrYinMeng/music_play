...
//fsize:整个文件大小
//pos:当前文件指针位置
u32 fupd_prog(u16 x,u16 y,u8 size,u32 fsize,u32 pos,u16 color)
{
	float prog;
	u8 t=0XFF;
	...
	if(t!=prog)
	{
		LCD_ShowString(x+3*size/2,y,240,320,size,"%",color);		
		t=prog;
		if(t>100)t=100;
		LCD_ShowNum(x,y,t,3,size,color);//显示数值
	}
	return 0;					    
} 
//更新某一个
...
//返回值:0,成功;其他,失败.
u8 updata_fontx(u16 x,u16 y,u8 size,u8 *fxpath,u8 fx,u16 color)
{
	u32 flashaddr=0;								    
	FIL * fftemp;
	u8 *tempbuf;
 	...
 	if(rval==0)	 
	{
		...
		while(res==FR_OK)//死循环执行
		{
			...
			W25QXX_Write(tempbuf,offx+flashaddr,4096);			//从0开始写入4096个数据  
	  		offx+=bread;	  
			fupd_prog(x,y,size,fftemp->obj.objsize,offx,color);	 	//进度显示
...
//返回值:0,更新成功;
//		 其他,错误代码.	  
u8 update_font(u16 x,u16 y,u8 size,u8* src,u16 color)
{	
	u8 *pname;
	u32 *buf;
	u8 res=0;		   
	...
	if(rval==0)//字库文件都存在.
	{  
		LCD_ShowString(x,y,240,320,size,"Erasing sectors... ",RED);//提示正在擦除扇区	
		for(i=0;i<FONTSECSIZE;i++)	//先擦除字库区域,提高写入速度
		{
			fupd_prog(x+20*size/2,y,size,FONTSECSIZE,i,color);//进度显示
		...
		myfree(SRAMIN,buf);
		LCD_ShowString(x,y,240,320,size,"Updating UNIGBK.BIN",RED);		
		strcpy((char*)pname,(char*)src);				//copy src内容到pname
		strcat((char*)pname,(char*)UNIGBK_PATH); 
		res=updata_fontx(x+20*size/2,y,size,pname,0,color);	//更新UNIGBK.BIN
		if(res){myfree(SRAMIN,pname);return 1;}
		LCD_ShowString(x,y,240,320,size,"Updating GBK12.BIN  ",RED);
		strcpy((char*)pname,(char*)src);				//copy src内容到pname
		strcat((char*)pname,(char*)GBK12_PATH); 
		res=updata_fontx(x+20*size/2,y,size,pname,1,color);	//更新GBK12.FON
		if(res){myfree(SRAMIN,pname);return 2;}
		LCD_ShowString(x,y,240,320,size,"Updating GBK16.BIN  ",RED);
		strcpy((char*)pname,(char*)src);				//copy src内容到pname
		strcat((char*)pname,(char*)GBK16_PATH); 
		res=updata_fontx(x+20*size/2,y,size,pname,2,color);	//更新GBK16.FON
		if(res){myfree(SRAMIN,pname);return 3;}
		LCD_ShowString(x,y,240,320,size,"Updating GBK24.BIN  ",RED);
		strcpy((char*)pname,(char*)src);				//copy src内容到pname
		strcat((char*)pname,(char*)GBK24_PATH); 
		res=updata_fontx(x+20*size/2,y,size,pname,3,color);	//更新GBK24.FON
